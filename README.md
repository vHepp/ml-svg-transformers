...
- **Structural Evaluation:** Evaluates the generated outputs based on strict formatting rules, testing for _XML validity_ (via `lxml`) and _renderability_ (via `CairoSVG`).
...
_Note: The datasets are cached locally. You may need to configure your `HF_HOME` environment variable as seen in the first cell of `main.ipynb` to direct the Hugging Face cache to your preferred directory._
